...
当前 Compose 始终启动 Kafka，并让 server 等待其健康；`KAFKA_ENABLED=false` 只关闭应用使用 Kafka，不会从 Compose 中移除服务。server 同样在 Compose 中固定 `LLM_ENABLED=false` 和 `CANAL_ENABLED=false`，因此当前一键路径即使 `KAFKA_ENABLED=true`，点赞/收藏仍使用本地提交后路径；完整 CDC/Kafka 互动模式需要另行适配并同时启用两个开关。存储默认 `local`，通过 `uploads_data` 供 server 写入、Nginx 读取。
