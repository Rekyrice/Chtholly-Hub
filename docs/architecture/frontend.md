...
用户主页的 `UserPage` 保持 Server Component：解析用户后并行加载文章首屏、关系计数和评论首屏，三类请求各自隔离失败，单个数据源异常不会阻断其余区域渲染。[`UserTabs`](../../apps/web/components/site/UserTabs.tsx) 在概览中展示最近 2 条评论，在评论标签页按页码继续加载（服务端采用 offset 分页）；失败页可 retry，合并分页结果时按评论 ID 稳定去重。用户切换通过按用户标识重挂载状态，并以 stale response guard 丢弃旧用户的迟到响应，避免跨用户混入评论。用户主页的“点赞”标签页仍沿用现有占位交互，不代表点赞列表接口已经实现。
...
- [`start-frontend.ps1`](../../scripts/dev/start-frontend.ps1) 会把仓库根 `.env` 注入 PowerShell 后再启动 Next.js；裸 `npm run dev` 与 `npm run build` 不会读取父目录根 `.env`，必须改用被忽略的 `apps/web/.env.local` 或终端/CI 进程环境。
...
运行 `build` 前同样要通过 `apps/web/.env.local` 或终端/CI 注入所需变量；仓库根 `.env` 不会被裸 npm 命令自动加载。
