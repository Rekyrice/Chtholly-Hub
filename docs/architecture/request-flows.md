...
- **异步/缓存**：`KAFKA_ENABLED=true` 时，Canal-compatible Outbox 消息经 `canal-outbox` 进入 [CounterReactionOutboxConsumer](../../apps/server/src/main/java/com/chtholly/counter/event/CounterReactionOutboxConsumer.java)；`false` 时，[CounterReactionLocalAdapter](../../apps/server/src/main/java/com/chtholly/counter/event/CounterReactionLocalAdapter.java) 在 `AFTER_COMMIT` 调用相同核心。[CounterReactionEventProcessor](../../apps/server/src/main/java/com/chtholly/counter/event/CounterReactionEventProcessor.java) 批量回查 MySQL 当前终态，用 Lua 幂等设置 Bitmap/SDS，再由 [CounterAggregationProcessor](../../apps/server/src/main/java/com/chtholly/counter/event/CounterAggregationProcessor.java) 写 Inbox 与同 epoch 快照。事件用于定位关系和携带增量/epoch，不覆盖 MySQL 终态。
