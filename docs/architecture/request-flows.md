...
- **同步主链**：Redis Lua 在同一次执行中读取维护 fence 与 fact epoch、验证两类分片索引哨兵及期望成员数、判断目标 bit，并只在状态真实变化时切换分片 Bitmap 与增减 SDS 即时计数；随后构造稳定 event ID 的 `CounterEvent`。SDS 缺失时先由 [CounterCalibrationService](../../apps/server/src/main/java/com/chtholly/counter/service/impl/CounterCalibrationService.java) 从两类 Bitmap 绝对校准，再重试一次；仍无法恢复时返回 503，不以零值掩盖故障。
- **异步/缓存**：`KAFKA_ENABLED=true` 时 [KafkaCounterPublisher](../../apps/server/src/main/java/com/chtholly/counter/event/KafkaCounterPublisher.java) 使用同一 event ID、key 和 payload 做有限 broker 确认重试；[CounterAggregationKafkaConsumer](../../apps/server/src/main/java/com/chtholly/counter/event/CounterAggregationKafkaConsumer.java) 批量消费并在成功后手动确认。`false` 时保留进程内 Spring Event 通道。[CounterAggregationProcessor](../../apps/server/src/main/java/com/chtholly/counter/event/CounterAggregationProcessor.java) 在一个 MySQL 事务中写事件 inbox、按实体/指标/fact epoch 合并并更新快照；浏览量仍使用 Redis 聚合桶与定时 SDS flush。
- **状态**：点赞/收藏成员事实只在 Redis Bitmap；SDS 是即时派生计数。MySQL `counter_event_inbox` 保存持久幂等事实，`counter_snapshot` 保存按 epoch 收敛的计数快照，两者都不能替代 Bitmap 成员事实。
- **失败/恢复**：Redis 与 Kafka 不共享事务，不能承诺 exactly-once。Lua 成功但 Kafka 未确认的窗口由周期校准发现：任务每轮严格推进一个持久 Bitmap SCAN cursor 页面；初始全量索引尚未完成时跳过实体校准，完成后再以有界实体批次同时处理 Redis 候选与 MySQL-only 旧快照。校准在 Redisson watchdog 锁保护下以无过期 token 阻断正常 toggle，绝对覆盖 SDS、清理旧 reaction 聚合、推进 fact epoch并写入同 epoch 的 MySQL 快照；进程崩溃留下的 token 在 watchdog 锁释放后由下一次 toggle 或校准安全接管，活跃锁仍返回 503。旧 Kafka 事件可以进入 inbox，但不能再次改变新 epoch 快照。Kafka 批次失败不确认，有限重试耗尽后只有在 DLT broker 确认成功后才恢复 offset。
- **代表性测试**：[CounterServiceImplBatchTest](../../apps/server/src/test/java/com/chtholly/counter/service/impl/CounterServiceImplBatchTest.java)、[CounterAggregationProcessorTest](../../apps/server/src/test/java/com/chtholly/counter/event/CounterAggregationProcessorTest.java)、[CounterCalibrationServiceTest](../../apps/server/src/test/java/com/chtholly/counter/service/impl/CounterCalibrationServiceTest.java)、[CounterFactMaintenanceLuaIT](../../apps/server/src/test/java/com/chtholly/integration/CounterFactMaintenanceLuaIT.java)、[CounterGoldenPathIT](../../apps/server/src/test/java/com/chtholly/integration/CounterGoldenPathIT.java)。
