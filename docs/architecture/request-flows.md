...
- **异步/缓存**：只有 `KAFKA_ENABLED=true` 且 `CANAL_ENABLED=true` 时，Canal-compatible Outbox 消息才经 `canal-outbox` 进入 [CounterReactionOutboxConsumer](../../apps/server/src/main/java/com/chtholly/counter/event/CounterReactionOutboxConsumer.java)；任一开关关闭时，[CounterReactionLocalAdapter](../../apps/server/src/main/java/com/chtholly/counter/event/CounterReactionLocalAdapter.java) 在 `AFTER_COMMIT` 调用相同核心。两个开关只负责选路，不探测 CDC 健康；选择 Kafka 路径前必须确认 CDC 可达，运行时没有自动本地回退。[CounterReactionEventProcessor](../../apps/server/src/main/java/com/chtholly/counter/event/CounterReactionEventProcessor.java) 批量回查 MySQL 当前终态，用 Lua 幂等设置 Bitmap/SDS，再由 [CounterAggregationProcessor](../../apps/server/src/main/java/com/chtholly/counter/event/CounterAggregationProcessor.java) 以 `REQUIRES_NEW` 提交 Inbox 与同 epoch 快照。事务返回后只发布当前处理批次内同关系键中 `eventId` 最大且与当前终态方向一致的事件；事件只用于定位关系和携带增量/epoch，不覆盖 MySQL 终态。
- **状态与读取**：`counter_reaction` 是成员关系唯一事实；Bitmap 是高频成员读投影，SDS 与 `counter_snapshot` 是计数投影，`counter_event_inbox` 只负责事件幂等。实体完整性标记只在一次全量重建完成后发布；标记有效时单条和 Feed 批量状态读取使用 Bitmap，缺失或结构不完整时批量回退 MySQL。已提交事实与投影之间允许短暂延迟。
- **失败/恢复**：关系提交不依赖 Redis 或 broker 成功；Kafka 路径用 5/30/120 秒的持久 retry envelope，并在 broker 确认转发后才 ACK；未到期 retry 通过 `nack` 保留原记录，可能形成分区头阻塞。本地路径异常只记录日志。投影失败由 Outbox 重放或 [CounterCalibrationService](../../apps/server/src/main/java/com/chtholly/counter/service/impl/CounterCalibrationService.java) 恢复；校准在 Redisson 锁与 token fence 下锁定 MySQL snapshot epoch，分页读取关系到 staging shard，并以有界 Lua 批次切换 Bitmap 索引、SDS、epoch 和完整性标记，失败不会把部分结果标成完整。重复 `eventId` 由 Inbox 去重，旧 epoch 延迟事件不能扰动新快照，但该链路不宣称 Kafka exactly-once。核心恢复只覆盖 Bitmap、SDS 与快照；通知、Feed/作者计数和兴趣画像没有逐监听器持久回执，进程崩溃、异步失败、部分监听器失败或守卫过期后的重放仍可能造成缺失或重复。
- **代表性测试**：[CounterReactionCommandServiceTest](../../apps/server/src/test/java/com/chtholly/counter/service/CounterReactionCommandServiceTest.java)、[CounterReactionEventProcessorTest](../../apps/server/src/test/java/com/chtholly/counter/event/CounterReactionEventProcessorTest.java)、[AbstractKafkaConsumerTest](../../apps/server/src/test/java/com/chtholly/common/kafka/AbstractKafkaConsumerTest.java)、[OutboxKafkaTopicConfigTest](../../apps/server/src/test/java/com/chtholly/relation/outbox/OutboxKafkaTopicConfigTest.java)、[CounterCalibrationServiceTest](../../apps/server/src/test/java/com/chtholly/counter/service/impl/CounterCalibrationServiceTest.java)、[CounterFactMaintenanceLuaIT](../../apps/server/src/test/java/com/chtholly/integration/CounterFactMaintenanceLuaIT.java)、[CounterReactionFactsIT](../../apps/server/src/test/java/com/chtholly/integration/CounterReactionFactsIT.java)、[CounterReactionLocalModeIT](../../apps/server/src/test/java/com/chtholly/integration/CounterReactionLocalModeIT.java)、[CounterReactionRebuildConcurrencyIT](../../apps/server/src/test/java/com/chtholly/integration/CounterReactionRebuildConcurrencyIT.java)、[CounterGoldenPathIT](../../apps/server/src/test/java/com/chtholly/integration/CounterGoldenPathIT.java)、[NotificationEventListenerTest](../../apps/server/src/test/java/com/chtholly/notification/listener/NotificationEventListenerTest.java)、[FeedCacheInvalidationListenerTest](../../apps/server/src/test/java/com/chtholly/post/listener/FeedCacheInvalidationListenerTest.java)、[UserInterestProfileListenerTest](../../apps/server/src/test/java/com/chtholly/recommendation/UserInterestProfileListenerTest.java)。后三个固定单监听器行为，真实 Spring 多播与同进程重放由 `CounterReactionLocalModeIT` 覆盖；这些测试不把进程崩溃或异步执行失败提升为持久可靠性承诺。
