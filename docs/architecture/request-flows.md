...
- **同步主链**：Redis Lua 在同一次执行中读取维护 fence 与 fact epoch、验证两类分片索引哨兵、判断目标 bit，并只在状态真实变化时切换分片 Bitmap 与增减 SDS 即时计数；随后构造稳定 event ID 的 `CounterEvent`。SDS 缺失时先由 [CounterCalibrationService](../../apps/server/src/main/java/com/chtholly/counter/service/impl/CounterCalibrationService.java) 从两类 Bitmap 绝对校准，再重试一次；仍无法恢复时返回 503，不以零值掩盖故障。
