...
- **一致性与降级**：缓存未命中通常回源 MySQL并回填；文章写入主动失效相关缓存。点赞/收藏 bitmap 无其他业务源，可靠性依赖 Redis 自身持久化与备份；丢失 bitmap 不能从 MySQL 或默认进程内事件恢复。身份验证、幂等位图、限流或聚合依赖 Redis 的路径在 Redis 不可用时可能直接失败。Agent 记忆另有进程内 Caffeine 热副本，但持久会话仍依赖 Redis。
