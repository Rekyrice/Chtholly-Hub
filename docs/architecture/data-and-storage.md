...
- **权威数据**：大多数业务最终事实在 MySQL；但点赞/收藏成员关系没有 MySQL 落点。当 `kafka.enabled=false` 时，分片 bitmap 是该成员关系唯一的持久状态源，进程内 Spring 事件不可重放。SDS 只是聚合计数：缺失时 [CounterServiceImpl](../../apps/server/src/main/java/com/chtholly/counter/service/impl/CounterServiceImpl.java) 对 bitmap 分片计数并回写 SDS。
