...
- **一致性与降级**：互动的 Kafka 与本地路径共用 MySQL 终态查询、Bitmap 投影和聚合组件；`CounterAggregationProcessor` 以 `REQUIRES_NEW` 提交 Inbox/快照后，才发布当前处理批次内同关系键中 `eventId` 最大且与当前终态方向一致的进程内事件。相同 `eventId` 由 `counter_event_inbox` 幂等，但这不等于 Kafka exactly-once。旧 epoch 的延迟事件可登记 Inbox，却不能修改新 epoch 快照；投影失败不回滚已提交关系，Outbox 重放或 MySQL 全量校准负责恢复 Bitmap、SDS 与快照。校准不补发通知、Feed/作者计数或兴趣画像，本地处理失败也没有自动持久重放；这些副作用仅由 24 小时 Redis 守卫抑制普通重放。`kafka.enabled=false` 时，浏览量等通用计数仍切到 [SpringEventCounterPublisher](../../apps/server/src/main/java/com/chtholly/counter/event/SpringEventCounterPublisher.java) 与 [CounterAggregationSpringConsumer](../../apps/server/src/main/java/com/chtholly/counter/event/CounterAggregationSpringConsumer.java)。浏览量的可选旧回放与 reaction 成员事实恢复严格分离。
