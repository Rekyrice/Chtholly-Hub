...
- **权威数据**：用户、文章元数据、标签、评论、关注写模型、通知、管理员审计、Outbox、Seed marker，以及可重建索引所需的源数据。MySQL 保存大多数业务最终事实，但点赞/收藏成员关系没有 MySQL 落点。
...
- **权威数据**：大多数业务最终事实在 MySQL；但点赞/收藏成员关系没有 MySQL 落点。无论 Kafka 是否启用，分片 bitmap 都是当前成员关系唯一的状态源。SDS 只是聚合计数：缺失时 [CounterServiceImpl](../../apps/server/src/main/java/com/chtholly/counter/service/impl/CounterServiceImpl.java) 对 bitmap 分片计数并回写 SDS。Kafka 中的点赞/收藏增量既不包含可恢复的完整成员快照，也不能跨越事实维护基线；[CounterRebuildConsumer](../../apps/server/src/main/java/com/chtholly/counter/event/CounterRebuildConsumer.java) 因此明确跳过这两类事件，不能替代 Redis 备份。Kafka 关闭时的额外差异是进程内 Spring 事件不可重放。
...
- **一致性与降级**：缓存未命中通常回源 MySQL并回填；文章写入主动失效相关缓存。点赞/收藏 bitmap 无其他业务源，可靠性依赖 Redis 自身持久化与备份；丢失 bitmap 不能从 MySQL 或默认进程内事件恢复。正式数据维护前使用 [`backup-redis.ps1`](../../scripts/backup/backup-redis.ps1) 在仓库外生成带 SHA-256 元数据的 RDB，并与 MySQL 备份保持同一维护窗口。身份验证、幂等位图、限流或聚合依赖 Redis 的路径在 Redis 不可用时可能直接失败。Agent 记忆另有进程内 Caffeine 热副本，但持久会话仍依赖 Redis。
...
- **一致性与降级**：`kafka.enabled=false` 时，计数切到 [SpringEventCounterPublisher](../../apps/server/src/main/java/com/chtholly/counter/event/SpringEventCounterPublisher.java) 与 [CounterAggregationSpringConsumer](../../apps/server/src/main/java/com/chtholly/counter/event/CounterAggregationSpringConsumer.java)，仍在进程内聚合但不可重放；`true` 时走 Kafka 生产者与消费者，同时发布本地计数事件供通知使用。[CounterRebuildConsumer](../../apps/server/src/main/java/com/chtholly/counter/event/CounterRebuildConsumer.java) 仅在 `kafka.enabled=true` 且 `counter.rebuild.enabled=true` 时装配，并且只允许对不承载成员关系的计数（当前主要是浏览量）做人工 best-effort 回放。该回放只能用于空目标，要求 Kafka retention 覆盖完整历史并显式使用新消费组或重置 offset；恢复完整 RDB 后禁止再叠加 earliest 回放。点赞/收藏必须恢复 Redis RDB/bitmap，SDS 缺失时再从 bitmap 派生。评论、关注通知和 Feed timeline 一直使用 Spring ApplicationEvent。Outbox 行仍会写入 MySQL，但关系 fan 表/ZSet 的 Outbox 消费没有等价的本地消费者；只有文章搜索写路径会额外尝试同步 ES。因此关闭 Kafka/Canal 不等于完整模拟 Outbox 下游。
