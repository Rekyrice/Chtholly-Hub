...
生产入口是 [`docker-compose.prod.yml`](../../docker-compose.prod.yml)：MySQL、Redis、Elasticsearch、Kafka、Spring Boot、Next.js standalone 与 Nginx 运行在同一 Compose 网络，只有 Nginx 暴露 `${HTTP_PORT:-80}`。默认正文存储为具名卷 `uploads_data`；需要 OSS 时必须同时调整存储配置和凭据。
...
需要自动 HTTPS 时使用示例 [`docker-compose.caddy.example.yml`](../../docker-compose.caddy.example.yml) 与 [`docker/caddy/Caddyfile`](../../docker/caddy/Caddyfile)，先替换示例域名和联系邮箱：

```bash
docker compose -f docker-compose.caddy.example.yml up -d --build
```

两套 Compose 是替代方案，不要同时占用 80/443。
