...
| [`deploy/`](deploy/) | `ecs-bootstrap.sh`、`ecs-ensure-swap.sh`、`ecs-init-db.sh`、`ecs-enable-https.sh` | 4 GiB 单机生产 Compose 首次部署、swap 检查、空库初始化与 HTTPS 安全切换 |
| [`deploy/`](deploy/) | `production-config.test.mjs` | 验证资源预算、日志轮转、低成本/Kafka profile、LLM/JWT、初始化与 HTTPS 配置契约 |
...
| [`benchmark/`](benchmark/) | `environment.ps1`、`run.ps1`、`seed.ps1`、`summarize.ps1`、`collect-counter-evidence.ps1`、`trace-replay.ps1` | 运行隔离缓存基准、采集互动计数正确性，或脱敏导出并比较固定 Agent Trace 回放 |
